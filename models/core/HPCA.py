...
class HPB(nn.Module):
    """Hybrid Perception Cross-Attention"""
    ...
    def __init__(
        ...
        # self.dwconv_ = nn.Conv2d(3 * dim, dim, 3, padding=1, groups=dim)
    ...
    def forward(self, x, qkv=None):
        attn_branch_out = self.attn(x, qkv)
        conv_branch_out = self.dwconv(x)
        # if qkv == None:
        #     conv_branch_out = self.dwconv(x)
        # else:
        #     q, k, v = qkv
        #     conv_branch_out = self.dwconv_(torch.cat((q, k, v), dim=1))
...
class DPSA(nn.Module):
    ...
    def forward(self, x, qkv=None):
        ...
        x = self.norm(x)
        if qkv is not None:
            q, k, v = map(lambda t: self.norm(t), qkv)
            q = self.to_q(q)
            k = self.to_q(k)
            v = self.to_q(v)
        else:
        ...
        q, k, v = map(lambda t: t.permute(0, 2, 3, 1), (q, k, v))
