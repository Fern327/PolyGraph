...
def get_cut_point(pred):
    room_polys = get_regions_from_pg(pred, corner_sorted=True)
    ori_corners=[tuple(p) for p in pred['corners']]
    remove_list=[]
    regions_corner=[]
    for r in room_polys:
        for p in r:
...
def render(corners, edges,edge_confs=None, render_pad=0, edge_linewidth=2, corner_size=3, scale=1.):
    size = int(256 * scale)
    mask = np.ones((2, size, size)) * render_pad

    corners = np.round(corners.copy() * scale).astype(np.int)
    for edge_i in range(edges.shape[0]):
        if edge_confs is not None and edge_confs[edge_i]<0.5:
            continue
        a = edges[edge_i, 0]
        b = edges[edge_i, 1]
